...
class JobService {
  ...
  async getGamePgn(jobId: string): Promise<string> {
    const res = await fetch(`${API_URL}/jobs/${jobId}/pgn`)
    if (!res.ok) {
  ...
  getCommentaryWsUrl(jobId: string): string {
    const wsBase = process.env.NEXT_PUBLIC_WS_URL || apiUrlToWsBase(API_URL)
